...
def fmt_size(byte):
    if byte >= 1024 * 1024 * 1024:
        return f'{byte / 1024 / 1024 / 1024 : .4f} GB'
    elif byte >= 1024 * 1024:
        return f'{byte / 1024 / 1024 : .4f} MB'
    elif byte >= 1024:
        return f'{byte / 1024 : .4f} KB'
    else:
        return f'{byte} Bytes'
...
def plain_rw_test(n, p):
    ...
    print(f'# 测试 {test_id}：n = {fmt_size(n)}, p = {p}, seed = {cur_seed}')
...
def broken_rw_test(n, p, idx, broke_type):
    ...
    print(
        f'# 测试 {test_id}：n = {fmt_size(n)}, p = {p}, idx = {idx}, seed = {cur_seed}')
    gen(n, testfile, cur_seed)
    write(testfile, p)
...
def repair_test(size, n, p, idx):
    ...
    print(
        f'# 测试 {test_id}：size = {size}, n = {fmt_size(n)}, p = {p}, idx = {idx}, seed = {cur_seed}')
...
def subtask_broken_rw():
    ...
    for n in [10, 100, 1000]:
        for p in [5, 7, 11]:
            broken_rw_test(n, p, [0], False)
            broken_rw_test(n, p, [1], True)
            broken_rw_test(n, p, [0, 1], False)
            broken_rw_test(n, p, [3, 5], False)

            broken_rw_test(n, p, [p], False)
            broken_rw_test(n, p, [p + 1], True)
            broken_rw_test(n, p, [p, p + 1], False)
            broken_rw_test(n, p, [3, p], True)
            broken_rw_test(n, p, [2, p + 1], False)
            reset()
